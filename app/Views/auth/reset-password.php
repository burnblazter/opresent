  ...
  <div class="page page-center">
    <div class="container container-tight py-4">
      ...
      <div class="card card-md">
        <div class="card-body">
          <h2 class="h2 text-center mb-3"><?= lang('Auth.resetYourPassword') ?></h2>
          ...
          <form action="<?= url_to('reset-password') ?>" method="post" autocomplete="off">
            <?= csrf_field() ?>
            <input type="hidden" name="token" value="<?= old('token', $token ?? '') ?>">

            <div class="mb-3">
              <label class="form-label"><?= lang('Auth.email') ?></label>
              <input type="email" class="form-control" value="<?= isset($email) ? esc($email) : '' ?>" disabled
                style="background-color: #e2e8f0; cursor: not-allowed; color: #64748b;">
              <small class="text-muted d-block mt-1">Email terdaftar untuk akun Anda</small>
            </div>
