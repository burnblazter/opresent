<?php
// \app\ThirdParty\MythAuth\Commands\Publish.php
...
class Publish extends BaseCommand
{
    ...
    /**
     * The path to App\ThirdParty\MythAuth\src directory.
     *
     * @var string
     */
    ...
    protected function publishModels()
    {
        ...
        foreach ($models as $model) {
            ...
            $content = $this->replaceNamespace($content, 'App\ThirdParty\MythAuth\Models', 'Models');
    ...
    protected function publishEntities()
    {
        ...
        $content = file_get_contents($path);
        $content = $this->replaceNamespace($content, 'App\ThirdParty\MythAuth\Entities', 'Entities');

        $content = str_replace("use CodeIgniter\\Entity;\n", '', $content);
        $content = str_replace("use App\\ThirdParty\\MythAuth\\Authorization\\GroupModel;\n", '', $content);
        $content = str_replace("use App\\ThirdParty\\MythAuth\\Authorization\\PermissionModel;\n", '', $content);

        $content = str_replace('extends Entity', 'extends \\App\\ThirdParty\\MythAuth\\Entities\\User', $content);
    ...
    protected function publishController()
    {
        ...
        $content = file_get_contents($path);
        $content = $this->replaceNamespace($content, 'App\ThirdParty\MythAuth\Controllers', 'Controllers');
    ...
    protected function publishView($view, string $prefix = '')
    {
        ...
        $content = file_get_contents($path);
        $content = str_replace('App\ThirdParty\MythAuth\Views', $namespace . '\Views\Auth', $content);
    ...
    protected function publishFilters()
    {
        ...
        foreach ($filters as $filter) {
            ...
            $content = file_get_contents($path);
            $content = $this->replaceNamespace($content, 'App\ThirdParty\MythAuth\Filters', 'Filters');
    ...
    protected function publishMigration()
    {
        ...
        foreach ($map as $file) {
            ...
            $content = $this->replaceNamespace($content, 'App\ThirdParty\MythAuth\Database\Migrations', 'Database\Migrations');
    ...
    protected function publishConfig()
    {
        ...
        $content = file_get_contents($path);
        $content = str_replace('namespace App\ThirdParty\MythAuth\Config', 'namespace Config', $content);
        $content = str_replace("use CodeIgniter\\Config\\BaseConfig;\n", '', $content);
        $content = str_replace('extends BaseConfig', 'extends \\App\\ThirdParty\\MythAuth\\Config\\Auth', $content);
